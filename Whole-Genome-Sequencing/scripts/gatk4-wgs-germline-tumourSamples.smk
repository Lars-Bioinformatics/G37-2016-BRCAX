...
PROJECT = "G37-2016"
NORMALS = [config[s]["tumor"] for s in config]
print(NORMALS)
...
rule all:
    input:
        ...
        expand("gatk4_haplotypeCaller/{project}_allTumourSamples_germline_variants.vcf.gz", project=PROJECT)
...
rule CombineGVCFs:
    ...
    output:
        gvcf="gvcf_files/{project}_allTumourSamples_variants_combined.g.vcf.gz"
    params:
...
rule GenotypeGVCFs_all:
    input:
        gvcf="gvcf_files/{project}_allTumourSamples_variants_combined.g.vcf.gz"
    output:
        vcf="gatk4_haplotypeCaller/{project}_allTumourSamples_germline_variants.vcf.gz"
    shell:
        """
        gatk --java-options {mem} GenotypeGVCFs \
        -R {ref} \
