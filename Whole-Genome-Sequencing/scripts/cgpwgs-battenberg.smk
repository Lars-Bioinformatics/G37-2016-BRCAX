...
# configfile: WORK+"samples_test.yaml"


# Resources - paths inside docker
ref = WORK+"reference_files/core_ref_GRCh37d5"
res = WORK+"reference_files"
...
PROTOCOL = "WGS"
CAVESPLIT = 350000
CONTIG_EXCLUDE = "NC_007605,hs37d5,GL%" # GRCh38: HLA*,chrUn*, GRCh37: NC_007605,hs37d5,GL%
...
# CNV_TOOL = "ascat" # ascat # battenberg
CNV_TOOL = "battenberg"
...
rule all:
    input:
        # [expand(OUTPUT+"/finished/SUCCESS_"+CNV_TOOL+"_{tumor}_vs_{normal}.txt", tumor=config[s]["tumor"], normal=config[s]["normal"]) for s in config]
        [expand(OUTPUT+"/{tumor}/timings/WGS_{tumor}_vs_{normal}.time.battenberg", tumor=config[s]["tumor"], normal=config[s]["normal"]) for s in config]
...
rule caveman_setup:
    ...
    output:
        res1_1=OUTPUT+"/{tumor}/timings/WGS_{CNV_TOOL}_{tumor}_vs_{normal}.time.CaVEMan_setup"
    threads: 24
    shell:
        """
        ...
        -e {CAVESPLIT} \
        -o {OUTPUT}/{wildcards.tumor}/{PROTOCOL}_{wildcards.tumor}/caveman_{CNV_TOOL} \
        -x {CONTIG_EXCLUDE} \
        -p setup' \
        2>&1 | tee {output}
        """
...
rule caveman_split:
    input:
        ...
        res1_3=OUTPUT+"/{tumor}/timings/WGS_{normal}.time.verify_WT"
    output:
        res2_1=OUTPUT+"/{tumor}/timings/WGS_{CNV_TOOL}_{tumor}_vs_{normal}.time.CaVEMan_split"
    threads: 24
    shell:
        """
...
checkpoint ascat:
    input:
        normal=INPUT_BAM+"/{normal}/{normal}.bam",
        tumor=INPUT_BAM+"/{tumor}/{tumor}.bam",
        res2_1=OUTPUT+"/{tumor}/timings/WGS_ascat_{tumor}_vs_{normal}.time.CaVEMan_split"
    output:
...
rule BRASS_input:
    input:
        ...
        tumor=INPUT_BAM+"/{tumor}/{tumor}.bam"
    output:
        res3_2=OUTPUT+"/{tumor}/timings/WGS_{CNV_TOOL}_{tumor}_vs_{normal}.time.BRASS_input"
    threads: 2
    shell:
        """
        ...
        -t {input.tumor} \
        -n {input.normal} \
        -o {OUTPUT}/{wildcards.tumor}/{PROTOCOL}_{wildcards.tumor}/brass_{CNV_TOOL} \
        -p input'
        2>&1 | tee {output}
        """
...
rule BRASS_cover:
    input:
        ...
        tumor=INPUT_BAM+"/{tumor}/{tumor}.bam"
    output:
        res3_3=OUTPUT+"/{tumor}/timings/WGS_{CNV_TOOL}_{tumor}_vs_{normal}.time.BRASS_cover"
    threads: 24
    shell:
        """
...
rule caveman:
    input:
        normal=INPUT_BAM+"/{normal}/{normal}.bam",
        tumor=INPUT_BAM+"/{tumor}/{tumor}.bam",
        res3_1=OUTPUT+"/{tumor}/timings/WGS_{tumor}_vs_{normal}.time."+CNV_TOOL,
    output:
        res4_2=OUTPUT+"/{tumor}/timings/WGS_{CNV_TOOL}_{tumor}_vs_{normal}.time.caveman"
    params:
        #samplestats=OUTPUT+"/{tumor}/WGS_{tumor}/ascat/{tumor}.samplestatistics.txt"
...
rule brass:
    input:
        normal=INPUT_BAM+"/{normal}/{normal}.bam",
        tumor=INPUT_BAM+"/{tumor}/{tumor}.bam",
        samplestats=OUTPUT+"/{tumor}/WGS_{tumor}/{CNV_TOOL}/{tumor}_vs_{normal}.samplestatistics.txt",
        res3_1=OUTPUT+"/{tumor}/timings/WGS_{tumor}_vs_{normal}.time.{CNV_TOOL}",
        res3_2=OUTPUT+"/{tumor}/timings/WGS_{CNV_TOOL}_{tumor}_vs_{normal}.time.BRASS_input",
        res3_3=OUTPUT+"/{tumor}/timings/WGS_{CNV_TOOL}_{tumor}_vs_{normal}.time.BRASS_cover"
    output:
        res5_1=OUTPUT+"/{tumor}/timings/WGS_{CNV_TOOL}_{tumor}_vs_{normal}.time.BRASS"
    threads: 24
    shell:
        """
...
rule cgpFlagCaVEMan:
    input:
        ...
        res4_2=OUTPUT+"/{tumor}/timings/WGS_{CNV_TOOL}_{tumor}_vs_{normal}.time.caveman",
    output:
        res5_3=OUTPUT+"/{tumor}/timings/WGS_{CNV_TOOL}_{tumor}_vs_{normal}.time.cgpFlagCaVEMan"
    params:
        # samplestats=OUTPUT+"/{tumor}/WGS_{tumor}/ascat/{tumor}.samplestatistics.txt",
        # NORM_CONTAM=lambda wildcards: get_normalContamination(OUTPUT+"/"+
        #     wildcards.tumor+"/WGS_"+wildcards.tumor+
...
rule caveman_annot:
    input:
        res5_3=OUTPUT+"/{tumor}/timings/WGS_{CNV_TOOL}_{tumor}_vs_{normal}.time.cgpFlagCaVEMan"
    output:
        res6_1=OUTPUT+"/{tumor}/timings/WGS_{CNV_TOOL}_{tumor}_vs_{normal}.time.CaVEMan_annot"
    threads: 24
    shell:
        """
        ...
        # annotate caveman
        rm -f {OUTPUT}/{wildcards.tumor}/{PROTOCOL}_{wildcards.tumor}/caveman_{CNV_TOOL}/{wildcards.tumor}_vs_{wildcards.normal}.annot.muts.vcf.gz*
        ...
        -i {OUTPUT}/{wildcards.tumor}/{PROTOCOL}_{wildcards.tumor}/caveman_{CNV_TOOL}/{wildcards.tumor}_vs_{wildcards.normal}.flagged.muts.vcf.gz \
        -o {OUTPUT}/{wildcards.tumor}/{PROTOCOL}_{wildcards.tumor}/caveman_{CNV_TOOL}/{wildcards.tumor}_vs_{wildcards.normal}.annot.muts.vcf' \
        2>&1 | tee {output}
        """
...
rule verifyBam_tumor:
    input:
        ...
        res3_1=OUTPUT+"/{tumor}/timings/WGS_{tumor}_vs_{normal}.time.{CNV_TOOL}"
    output:
        res6_2=OUTPUT+"/{tumor}/timings/WGS_{CNV_TOOL}_{tumor}_vs_{normal}.time.verify_MT"
    threads: 24
    shell:
        """
        ...
        cgpwgs \
        bash -c '/usr/bin/time -v verifyBamHomChk.pl -d 25 \
        -o {OUTPUT}/{wildcards.tumor}/{PROTOCOL}_{wildcards.tumor}/contamination_{CNV_TOOL} \
        -b {input.tumor} \
        -t {threads} \
        -a {OUTPUT}/{wildcards.tumor}/{PROTOCOL}_{wildcards.tumor}/{CNV_TOOL}/{wildcards.tumor}.copynumber.caveman.csv \
        -j {OUTPUT}/{wildcards.tumor}/{PROTOCOL}_{wildcards.tumor}/contamination_{CNV_TOOL}/result.json \
        -s {res}/qcGenotype/verifyBamID_snps.vcf.gz' \
        2>&1 | tee {output}
        """
...
checkpoint battenberg:
    input:
        ...
        tumor=INPUT_BAM+"/{tumor}/{tumor}.bam",
    output:
        res3_1=OUTPUT+"/{tumor}/timings/WGS_{tumor}_vs_{normal}.time.battenberg",
        samplestats=OUTPUT+"/{tumor}/WGS_{tumor}/battenberg/{tumor}_vs_{normal}.samplestatistics.txt"
    threads: 24
    shell:
        """
        python2 ~/udocker-1.1.4/udocker run \
        ...
        cat {OUTPUT}/{wildcards.tumor}/{PROTOCOL}_{wildcards.tumor}/battenberg/{wildcards.tumor}_{{normal_contamination,cellularity_ploidy,rho_and_psi}}.txt | \
        awk -F'\t' '{{ \
        ...
        {OUTPUT}/{wildcards.tumor}/WGS_{wildcards.tumor}/battenberg/{wildcards.tumor}.copynumber.caveman.csv
        """
        # -ge L \
        # -gl {res}/qcGenotype/gender.tsv \
...
rule run_parallel:
    ...
    output:
        OUTPUT+"/finished/SUCCESS_{CNV_TOOL}_{tumor}_vs_{normal}.txt"
    shell:
        """
