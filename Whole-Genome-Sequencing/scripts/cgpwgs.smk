...
rule all:
    input:
        [expand(OUTPUT+"/finished/SUCCESS_"+CNV_TOOL+"__{tumor}_vs_{normal}.txt", tumor=config[s]["tumor"], normal=config[s]["normal"]) for s in config]
...
rule run_parallel:
    ...
    output:
        OUTPUT+"/finished/SUCCESS_{CNV_TOOL}__{tumor}_vs_{normal}.txt"
    shell:
        """
